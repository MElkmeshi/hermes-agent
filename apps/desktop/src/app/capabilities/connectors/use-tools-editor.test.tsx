...
describe('the editor machine', () => {
  ...
  it('stops on a conflict and keeps the edit until the reader chooses', async () => {
    ...
    // "Save over their version" is a save, so it carries the overwrite in the call itself.
  ...
  it('carries an organisation-locked slug across a quick action instead of dropping it', () => {
    // The org already turned it off, so no quick action may put it back.
  ...
  it('reloads for another connector even when the two saved rules read the same', () => {
    ...
    // Equal by value, new by identity: a store publish rebuilds both arrays and must not wipe an edit.
    ...
    // The saved rule moving underneath the same editor reloads it too.
    rerender({
      editorKey: 'github',
      onSave: async () => 'saved' as const,
      savedDisabled: ['LINEAR_ARCHIVE_ISSUE'],
      tools
    })
